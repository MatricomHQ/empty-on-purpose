...
class FractalTag2 {
  ...
  uint32_t parent_kf_; // kf that parent used to route to this child (0 for root)
  ...
  void double_dir() {
    ...
    for (uint32_t i = 0; i < ds_sz_; i++) {
      ...
      if (is_child(slot)) {
        ...
        if (child->first_idx_ == i)
          child->first_idx_ = 2 * i;
        if (child->last_idx_ == i)
          child->last_idx_ = 2 * i + 1;
      } else {
  ...
  bool spawn_child(uint32_t dx) {
    ...
    child->parent_kf_ = entry_kf(old->entries[0]);
...
public:
  FractalTag2(UArena &ua, std::atomic<uint64_t> *cnt, uint32_t disc_off = 0,
      ...
      : ua_(ua), count_(cnt), disc_off_(disc_off), max_gd_(max_gd),
        parent_kf_(0), first_idx_(0), last_idx_(0) {
    gd_ = 0;
    ds_sz_ = 1;
    shift_ = 32;
  ...
  bool insert_internal(const uint8_t *k, uint32_t kl, uint32_t v,
    ...
    for (int a = 0; a < 256; a++) {
      ...
      if (is_child(slot)) {
        FractalTag2 *child = get_child(slot);
        // CRITICAL: Verify kf matches what the child expects.
        // All entries in this child share parent_kf_ at the parent's level.
        // If our kf differs, we must NOT descend — grow the parent directory
        // until we get our own slot.
        if (child->parent_kf_ != kf) {
          // Grow directory and split child's slot range: keep child on
          // matching half, install empty group on non-matching half.
          // Find the first bit where child's kf and our kf differ
          uint32_t xor_bits = child->parent_kf_ ^ kf;
          uint8_t split_bit = __builtin_clz(xor_bits); // first differing bit
          // Grow directory until gd > split_bit
          while (node->gd_ <= split_bit) {
            node->double_dir();
          }
          // Now carve out the child's current slot range and split it
          // Find child's current stride based on its last_idx_ - first_idx_
          uint32_t child_fi = child->first_idx_;
          uint32_t child_li = child->last_idx_;
          uint32_t child_stride = child_li - child_fi + 1;
          // The bit we're splitting on: split_bit relative to gd
          // Half the stride
          uint32_t half = child_stride / 2;
          if (half == 0) half = 1;
          uint8_t child_side = (child->parent_kf_ >> (31 - split_bit)) & 1;
          // Create empty group for the non-child side
          Group *empty = node->alloc_group(split_bit + 1);
          if (child_side == 0) {
            // Child keeps first half, empty group gets second half
            child->last_idx_ = child_fi + half - 1;
            empty->first_idx_ = child_fi + half;
            empty->last_idx_ = child_li;
            for (uint32_t i = child_fi + half; i <= child_li; i++)
              node->dir_[i] = (uintptr_t)empty;
          } else {
            // Child keeps second half, empty group gets first half
            child->first_idx_ = child_fi + half;
            empty->first_idx_ = child_fi;
            empty->last_idx_ = child_fi + half - 1;
            for (uint32_t i = child_fi; i < child_fi + half; i++)
              node->dir_[i] = (uintptr_t)empty;
          }
          // Recompute dx and retry
          continue;
        }
        node = child;
        node->extract_at(k, kl, kf, tag);
