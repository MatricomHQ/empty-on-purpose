...
public:
  ...
  bool spawn_child(uint32_t dx) {
    ...
    if (child_off <= disc_off_ || child_off >= min_kl)
      return false;

    uint32_t occ = ~scan32(old->tags, 0);  // occupied bitmask

    // ── In-place prefix compression ──
    if (ld == 0) {
      uint32_t probe_diff = 0;
      uint32_t kf0_new;
      int first = __builtin_ctz(occ);
      extract_kf(arena_.rec_key(old->offs[first]), arena_.rec_kl(old->offs[first]),
                  child_off, kf0_new);
      uint32_t tmp = occ & (occ - 1);
      while (tmp) {
        int idx = __builtin_ctz(tmp);
        uint32_t kfi;
        extract_kf(arena_.rec_key(old->offs[idx]), arena_.rec_kl(old->offs[idx]),
                    child_off, kfi);
        probe_diff |= kf0_new ^ kfi;
        tmp &= tmp - 1;
      }
      if (probe_diff != 0 && __builtin_clz(probe_diff) < max_gd_) {
        uint32_t saved_count = old->count;
        uint32_t saved_offs[kCap];
        uint32_t si = 0;
        uint32_t tmp2 = occ;
        while (tmp2) {
          int idx = __builtin_ctz(tmp2);
          saved_offs[si++] = old->offs[idx];
          tmp2 &= tmp2 - 1;
        }
        ...
        old->first_idx = 0;
        old->last_idx = 0;
        memset(old->tags, 0, kCap);

        count_->fetch_sub(saved_count, std::memory_order_relaxed);
        for (uint32_t i = 0; i < saved_count; i++) {
          uint32_t off = saved_offs[i];
          insert_internal(arena_.rec_key(off), arena_.rec_kl(off),
