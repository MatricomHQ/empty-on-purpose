...
namespace x86 {
    ...
    inline void cmp_rsi_imm(uint8_t* d, size_t& n, uint32_t v) {
        if (v < 128) {
            d[n++] = 0x48; d[n++] = 0x83; d[n++] = 0xFE; d[n++] = (uint8_t)v;
        } else {
            d[n++] = 0x48; d[n++] = 0x81; d[n++] = 0xFE;
            memcpy(&d[n], &v, 4); n += 4;
        }
    }
    ...
    inline void mov_al_rdi_off(uint8_t* d, size_t& n, uint32_t off) {
        if (off == 0) {
            ...
            d[n++] = 0x8A; d[n++] = 0x47; d[n++] = (uint8_t)off;
        } else {
            d[n++] = 0x8A; d[n++] = 0x87;
            memcpy(&d[n], &off, 4); n += 4;
        }
    }
    ...
    inline void mov_rdx_rdi_off(uint8_t* d, size_t& n, uint32_t off) {
        if (off == 0) {
            d[n++] = 0x48; d[n++] = 0x8B; d[n++] = 0x17;
        } else if (off < 128) {
            d[n++] = 0x48; d[n++] = 0x8B; d[n++] = 0x57; d[n++] = (uint8_t)off;
        } else {
            d[n++] = 0x48; d[n++] = 0x8B; d[n++] = 0x97;
            memcpy(&d[n], &off, 4); n += 4;
        }
    }
...
struct Arena {
    ...
    Arena(size_t capacity) : cap(capacity) {
        mem = (uint8_t*)mmap(nullptr, capacity, PROT_READ|PROT_WRITE, MAP_PRIVATE|MAP_ANONYMOUS, -1, 0);
    }
...
struct FractalJITTrie {
    ...
    FractalJITTrie(size_t arena_cap, size_t code_cap) : arena(arena_cap), jit_cap(code_cap) {
        ...
        jit_mem = mmap(nullptr, code_cap, PROT_READ|PROT_WRITE|PROT_EXEC, MAP_PRIVATE|MAP_ANONYMOUS, -1, 0);
    ...
    void insert(Node** node_ptr, const char* k, size_t l, uint64_t h, int depth) {
        ...
        while (left <= right) {
            ...
            if (mid_c == c) {
                ...
                int match_len = 0;
                while (depth + match_len < l && depth + match_len < child->len && k[depth + match_len] == child->key[depth + match_len]) {
                    match_len++;
                }
                ...
                Node* split = (Node*)arena.alloc(sizeof(Node) + child->num_children * sizeof(Node*));
                split->key = child->key;
                split->len = child->len;
                split->handle = child->handle;
                split->num_children = child->num_children;
                memcpy(split->children(), child->children(), child->num_children * sizeof(Node*));
                ...
                if (depth + match_len == l) {
                    ...
                    Node* new_child = (Node*)arena.alloc(sizeof(Node) + 2 * sizeof(Node*));
                    new_child->key = child->key;
                    new_child->len = child->len;
                    new_child->handle = child->handle;
        ...
        Node* new_node = (Node*)arena.alloc(sizeof(Node) + (node->num_children + 1) * sizeof(Node*));
        ...
        memcpy(new_children + left + 1, children + left, (node->num_children - left) * sizeof(Node*));
    ...
    __attribute__((always_inline)) inline uint64_t lookup(const char* key, size_t len) const {
        return fn(key, len);
    }
...
struct BR { double mean, p50, p95; };
template<typename Fn>
BR bench_fn(Fn&& fn, size_t batch, size_t samples) {
    for (size_t i = 0; i < 2000; i++) fn();
...
int main() {
    ...
    double umap_ins_ms = std::chrono::duration<double, std::milli>(t1_umap_ins - t0_umap_ins).count();
    std::cout << "std::unordered_map Insert Time: " << umap_ins_ms << " ms" << std::endl;

    FractalJITTrie trie(1024 * 1024 * 256, 1024 * 1024 * 128); // 256MB arena, 128MB code
    ...
    double jit_ins_ms = std::chrono::duration<double, std::milli>(t1_jit_ins - t0_jit_ins).count();
    ...
    printf(" → Fractal: %.2fx %s than unordered_map\n",
           sf > 1 ? sf : 1 / sf, sf > 1 ? "FASTER" : "SLOWER");
